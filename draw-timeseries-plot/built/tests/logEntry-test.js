...
let visionDatas = new Array();
let entries = new Array();
lines.forEach((line) => {
    var entry = logEntry_1.getLogEntry(line);
    entries.push(entry);
    if (entry.getEntryType() == logEntry_1.EntryType.Score) {
        scores.push(entry);
    }
    else if (entry.getEntryType() == logEntry_1.EntryType.VisionData) {
...
if (scores.length > 0) {
    console.log("Timestamp: %s, EntryType: %s, MomentTimestamp: %s, Score: %s", scores[0].TimeStamp, scores[0].getEntryType(), scores[0].getMomentTimestamp(), scores[0].getValue());
}
if (visionDatas.length > 0) {
    console.log("Timestamp: %s, EntryType: %s, MomentTimestamp: %s, VisionData: %s", visionDatas[0].TimeStamp, visionDatas[0].getEntryType(), visionDatas[0].getMomentTimestamp(), visionDatas[0].getValue());
}
console.log("Done.");
//# sourceMappingURL=logEntry-test.js.map